...
class _ASPPModule(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pyramids):
        ...
        self.stages = nn.ModuleList([
            ...
            for dilation, padding in zip(pyramids, pyramids)
        ])
...
class DeepLabV2(nn.Sequential):
    ...
    def __init__(self, n_classes, n_blocks, pyramids):
        ...
        self.add_module('layer1', nn.Sequential(OrderedDict([
        ...
        self.add_module('layer5', _ResBlock(n_blocks[3], 1024, 512, 2048, 1, 4))  # NOQA
...
class MSC(nn.Module):
    ...
    def forward(self, x):
        ...
        size075 = int(input_size * 0.75) + 1
        size050 = int(input_size * 0.5) + 1
        ...
        output075 = self.interp100(self.scale(self.interp075(x)))
        output050 = self.interp100(self.scale(self.interp050(x)))
...
def DeepLabV2_ResNet101_MSC(n_classes):
    return MSC(DeepLabV2(n_classes=n_classes,
                         n_blocks=[3, 4, 23, 3],
                         pyramids=[6, 12, 18, 24]))
